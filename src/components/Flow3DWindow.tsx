...
export function Flow3DWindow({
  ...
  const handleCameraControl = (mode: 'orbit' | 'zoom') => {
    ...
    addToast(
      'Camera animation: ' + (mode === 'orbit' ? 'Orbit mode' : 'Zoom mode') + ' activated',
      "info"
    );
        ...
        >
          <div className="window-header flex items-center justify-between p-2 bg-muted/50 border-b cursor-move">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleCameraControl('orbit')} title="Orbit Camera">
                <Camera className={cn("w-3 h-3", cameraAnimation === 'orbit' && "text-primary animate-pulse")} />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleApplyLayout} title="Optimize Layout">
                <Wand2 className="w-3 h-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6 window-button cursor-pointer" onClick={onMinimize}>
                <Minus className="w-3 h-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6 window-button cursor-pointer" onClick={handleMaximize}>
                <Maximize2 className="w-3 h-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6 window-button cursor-pointer" onClick={onClose}>
                <X className="w-3 h-3" />
              </Button>
            </div>
          </div>
